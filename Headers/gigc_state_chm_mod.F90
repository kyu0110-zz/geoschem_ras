...
!BOC
    ...
    print*, 'MAX_DEP', max_dep
    print*, 'n_tracers', nTracers
    ...
    ALLOCATE( State_Chm%Spec_Id       (             nSpeciesAlloc   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN

    ALLOCATE( State_Chm%Spec_Name     (             nSpeciesAlloc   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN

    ALLOCATE( State_Chm%Species       ( IM, JM, LM, nSpeciesAlloc   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
