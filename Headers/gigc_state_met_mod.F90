...
!
  ...
  TYPE, PUBLIC :: MetState
     ...
     REAL(fp), POINTER :: PBLH_MAX  (:,:  ) ! PBL height [m]
...
!                                            !  5 hrs ago (for PARANOX)
     ...
     REAL(fp), POINTER :: RAS_CMFMC (:,:,:) ! Cloud mass flux [kg/m2/s]
     ...
     REAL(fp), POINTER :: RAS_DQRCU (:,:,:) ! Conv precip prod rate [kg/kg/s]
     REAL(fp), POINTER :: DQRCU     (:,:,:) ! Conv precip prod rate [kg/kg/s]
                                            ! (assume per dry air)
     REAL(fp), POINTER :: RAS_DQRLSAN(:,:,:) ! LS precip prod rate [kg/kg/s]
     ...
     REAL(fp), POINTER :: RAS_DTRAIN(:,:,:) ! Detrainment flux [kg/m2/s]
     ...
     REAL(fp), POINTER :: MOISTQ    (:,:,:) ! Tendency in sp. humidity 
                                            ! [kg/kg moist air/s]
     REAL(fp), POINTER :: OMEGA     (:,:,:) ! Vertical net mass flux [Pa s^-1]
     REAL(fp), POINTER :: NEGOMEGA  (:,:,:) ! Vertical down mass flux [Pa s^-1]
     ...
     REAL(fp), POINTER :: RAS_PFICU (:,:,:) ! Dwn flux ice prec:conv [kg/m2/s]
     REAL(fp), POINTER :: PFILSAN   (:,:,:) ! Dwn flux ice prec:LS+anv [kg/m2/s]
     REAL(fp), POINTER :: RAS_PFILSAN(:,:,:) ! Dwn flux ice prec:LS+anv [kg/m2/s]
     REAL(fp), POINTER :: RAS_PFLCU (:,:,:) ! Dwn flux liq prec:conv [kg/m2/s]
     REAL(fp), POINTER :: PFLCU     (:,:,:) ! Dwn flux liq prec:conv [kg/m2/s]
     REAL(fp), POINTER :: PFLLSAN   (:,:,:) ! Dwn flux ice prec:LS+anv [kg/m2/s]
     REAL(fp), POINTER :: RAS_PFLLSAN(:,:,:) ! Dwn flux ice prec:LS+anv [kg/m2/s]
     ...
     REAL(fp), POINTER :: RAS_REEVAPCN(:,:,:) ! Evap of precip conv [kg/kg/s]
     REAL(fp), POINTER :: REEVAPCN  (:,:,:) ! Evap of precip conv [kg/kg/s]
                                            ! (assume per dry air)
     REAL(fp), POINTER :: RAS_REEVAPLS(:,:,:) ! Evap of precip LS+anvil [kg/kg/s]
...
!
    ...
    ALLOCATE( State_Met%EVAP      ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%GRN       ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%GRN      = 0.0_fp 

    ALLOCATE( State_Met%GWETROOT  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%GWETROOT = 0.0_fp 

    ALLOCATE( State_Met%GWETTOP   ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%LAI       ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%PARDR     ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%PARDR    = 0.0_fp

    ALLOCATE( State_Met%PARDF     ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%PBLH_MAX  ( IM, JM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%PBLH_MAX = 0.0_fp

    ALLOCATE( State_Met%PBL_TOP_L ( IM, JM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%PBL_TOP_L = 0

    ALLOCATE( State_Met%PHIS      ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%PRECSNO   ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%RADLWG    ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%SLP       ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%SLP      = 0.0_fp

    ALLOCATE( State_Met%SNODP     ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%SNODP    = 0.0_fp

    ALLOCATE( State_Met%SNOMAS    ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
...
!    State_Met%SUNCOSmid5 = 0.0_fp
    ...
    ALLOCATE( State_Met%TO3       ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%TSKIN     ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    !=======================================================================
    ALLOCATE( State_Met%FRSEAICE  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%PRECANV   ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%PRECANV  = 0.0_fp

    ALLOCATE( State_Met%PRECLSC   ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%PRECLSC  = 0.0_fp

    ALLOCATE( State_Met%SEAICE00  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%SEAICE00 = 0.0_fp

    ALLOCATE( State_Met%SEAICE10  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%SEAICE10 = 0.0_fp

    ALLOCATE( State_Met%SEAICE20  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%SEAICE20 = 0.0_fp

    ALLOCATE( State_Met%SEAICE30  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%SEAICE30 = 0.0_fp

    ALLOCATE( State_Met%SEAICE40  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%SEAICE40 = 0.0_fp

    ALLOCATE( State_Met%SEAICE50  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%SEAICE50 = 0.0_fp

    ALLOCATE( State_Met%SEAICE60  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%SEAICE60 = 0.0_fp

    ALLOCATE( State_Met%SEAICE70  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%SEAICE70 = 0.0_fp

    ALLOCATE( State_Met%SEAICE80  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%SEAICE80 = 0.0_fp

    ALLOCATE( State_Met%SEAICE90  ( IMreduced, JMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
...
!    ALLOCATE( State_Met%AREA_M2   ( IM, JM, LM   ), STAT=RC )
    ...
    ALLOCATE( State_Met%CLDF      ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    ...
    ALLOCATE( State_Met%RAS_CMFMC     ( IM, JM, LM+1 ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    State_Met%RAS_CMFMC    = 0.0_fp                                          
    ...
    ALLOCATE( State_Met%RAS_DQRCU ( IM, JM, LM   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%RAS_DQRCU = 0.0_fp

    ALLOCATE( State_Met%DQRLSAN   ( IM, JM, LM   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%DQRLSAN  = 0.0_fp

    ALLOCATE( State_Met%RAS_DQRLSAN (IM, JM, LM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%RAS_DQRLSAN = 0.0_fp

    ALLOCATE( State_Met%DQIDTMST  ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    State_Met%DQIDTMST = 0.0_fp
                                               
    ALLOCATE( State_Met%DQLDTMST  ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    State_Met%DQLDTMST = 0.0_fp
                                               
    ALLOCATE( State_Met%DQVDTMST  ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    ...
    ALLOCATE( State_Met%RAS_DTRAIN    ( IM, JM, LM   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    State_Met%RAS_DTRAIN   = 0.0_fp

    ALLOCATE( State_Met%MOISTMW ( IM, JM, LM   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    State_Met%MOISTMW = 0.0_fp
                                               
    ALLOCATE( State_Met%MOISTQ    ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    ...
    ALLOCATE( State_Met%NegOmega  ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%NegOmega = 0.0_fp
                                               
    ALLOCATE( State_Met%OPTD      ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    State_Met%OPTD     = 0.0_fp
                                               
    ALLOCATE( State_Met%OPTDEP    ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%PV        ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%PV       = 0.0_fp

    ALLOCATE( State_Met%QI        ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    State_Met%QI       = 0.0_fp
                                               
    ALLOCATE( State_Met%QL        ( IMreduced, JMreduced, LMreduced ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    State_Met%QL       = 0.0_fp
    ...
    ALLOCATE( State_Met%TAUCLI    ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN           
    State_Met%TAUCLI   = 0.0_fp
                                               
    ALLOCATE( State_Met%TAUCLW    ( IMreduced, JMreduced, LMreduced   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%UPDVVEL   ( IMreduced, JMreduced , LMreduced    ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    ...
    ALLOCATE( State_Met%RAS_PFICU ( IM, JM, LX   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%RAS_PFICU= 0.0_fp

    ALLOCATE( State_Met%PFILSAN   ( IM, JM, LX   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%PFILSAN  = 0.0_fp

    ALLOCATE( State_Met%RAS_PFILSAN( IM, JM, LX   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%RAS_PFILSAN = 0.0_fp

    ALLOCATE( State_Met%PFLCU     ( IM, JM, LX   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%PFLCU    = 0.0_fp

    ALLOCATE( State_Met%RAS_PFLCU ( IM, JM, LX   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%RAS_PFLCU = 0.0_fp

    ALLOCATE( State_Met%PFLLSAN   ( IM, JM, LX   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%PFLLSAN  = 0.0_fp

    ALLOCATE( State_Met%RAS_PFLLSAN   ( IM, JM, LX   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%RAS_PFLLSAN  = 0.0_fp

    ALLOCATE( State_Met%REEVAPCN  ( IM, JM, LM   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%REEVAPCN = 0.0_fp

    ALLOCATE( State_Met%RAS_REEVAPCN  ( IM, JM, LM   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%RAS_REEVAPCN = 0.0_fp

    ALLOCATE( State_Met%REEVAPLS  ( IM, JM, LM   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%REEVAPLS = 0.0_fp

    ALLOCATE( State_Met%RAS_REEVAPLS  ( IM, JM, LM   ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%RAS_REEVAPLS = 0.0_fp
...
!
    ...
    IF ( ASSOCIATED( State_Met%PBLH_MAX   )) DEALLOCATE( State_Met%PBLH_MAX   )
...
!    IF ( ASSOCIATED( State_Met%SUNCOSmid5 )) DEALLOCATE( State_Met%SUNCOSmid5 )
    ...
    IF ( ASSOCIATED( State_Met%NegOmega   )) DEALLOCATE( State_Met%NegOmega   )
