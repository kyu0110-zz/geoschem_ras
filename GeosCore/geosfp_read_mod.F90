...
!
  ...
  PRIVATE :: GeosFp_Read_A3cld_native
  PRIVATE :: GeosFp_Read_A3dyn
  PRIVATE :: GeosFp_Read_A3dyn_native
  PRIVATE :: GeosFp_Read_A3mstC
  PRIVATE :: GeosFp_Read_A3mstC_native
  PRIVATE :: GeosFp_Read_A3mstE
  PRIVATE :: GeosFp_Read_A3mstE_native
...
! 
  ...
  PUBLIC  :: GeosFp_Read_OMEGA_native
...
!
    ...
    nc_file = 'GEOSFP.YYYYMMDD.CN.Native.nc' 
#else
...
!
    USE GIGC_Input_Opt_Mod, ONLY : OptInput
    USE GIGC_State_Met_Mod, ONLY : MetState
    USE REGRID_A2A_MOD,     ONLY : READ_INPUT_GRID
...
!
    ...
    INTEGER            :: fId                ! netCDF file ID
    INTEGER            :: X, Y, T            ! netCDF file dimensions
    INTEGER            :: I, J, II, JJ       ! Do-loop indices
    ...
    CHARACTER(LEN=255) :: errMsg             ! Error message
    CHARACTER(LEN=255) :: caller             ! Name of this routine
    CHARACTER(LEN=255) :: gridfile           ! high-res file
    REAL*4             :: maxpbl
    ...
    INTEGER            :: st3d(3), ct3d(3)   ! Start + count, for 3D arrays 
    REAL*4             :: Q(IIPAR,JJPAR)     ! Temporary data arrray
    REAL*4             :: QHR(1152,721)     ! Temporary data arrray
    REAL*4             :: Qmid(IIPAR, 721)  ! Temporary data arrray
    REAL*8             :: LON_EDGES(1153)
    REAL*8             :: LAT_SINES(722)

    ! Logical for Rn simulation where we read in fewer variables
 ...
 20    FORMAT( '     - GEOS-FP A1 met fields for ', a,  &
    ...
    ! Get variable for we are doing a RnPbBe simulation
 ...
 100   FORMAT( 'Time_index value ', i5, ' must be in the range 1 to 24!' )
    ...
    ! If RnPbBe simulation we do not read all met
    ! in order to reduce memory for global simulation
    IF (ITS_A_RnPbBe_SIM) THEN 
    State_Met%CLDFRC = 0
    State_Met%EFLUX = 0
    State_Met%EVAP = 0
    State_Met%FRSEAICE = 0
    State_Met%GRN = 0
    State_Met%GWETROOT = 0
    State_Met%GWETTOP = 0
    State_Met%LAI = 0
    State_Met%RADLWG = 0
    State_Met%PARDF = 0
    ...
    State_Met%PRECSNO = 0
    State_Met%PRECLSC = 0
    State_Met%SEAICE00 = 0
    State_Met%SEAICE10 = 0
    State_Met%SEAICE20 = 0
    State_Met%SEAICE30 = 0
    State_Met%SEAICE40 = 0
    State_Met%SEAICE50 = 0
    State_Met%SEAICE60 = 0
    State_Met%SEAICE70 = 0
    State_Met%SEAICE80 = 0
    State_Met%SEAICE90 = 0
    State_Met%SLP = 1013.5
    State_Met%SNODP = 0
    State_Met%SNOMAS = 0
    State_Met%TO3 = 0
 ...
 10 FORMAT( '     - Found all 44 GEOS-FP A1     met fields for ', a )
    ...
    ! regrid PBLH by taking max height
    print*, 'reading high-resolution pblh'
    ! Replace time & date tokens in the file name
    dir     = TRIM( '/n/seasasfs02/kyu/c360/GEOS_0.25x0.3125/GEOS_FP/YYYY/MM/' )
    CALL EXPAND_DATE( dir, YYYYMMDD, HHMMSS )

    ! Replace time & date tokens in the file name
    nc_file = 'GEOSFP.YYYYMMDD.A1.Native.nc' 
    CALL EXPAND_DATE( nc_file, YYYYMMDD, HHMMSS )
    ...
    ! Make sure the dimensions of the file are valid
    !CALL Check_Dimensions( lon=X, lat=Y, lev=Z, time=T,         &
    !                        time_expected=8, caller=caller )

    ! Read data from file
    IF ( time_index < 1 .or. time_index > T ) THEN
        WRITE( 6, 200 ) time_index
200     FORMAT( 'Time_index_value', i5, ' must be in the range 1 to 8!' )
        CALL Error_Stop( errMsg, caller )
    ENDIF
    ...
    v_name = 'PBLH'
    CALL NcRd( QHR, fId, TRIM(v_name), st3d, ct3d )
    
    ! Echo info
    stamp = TimeStamp_String( YYYYMMDD, HHMMSS )
    WRITE( 6, 30 ) stamp
 30 FORMAT('               - Found HR GEOS-FP PBLH met fields for ', a )


    print*, 'Regridding by taking max'

    gridfile = TRIM('/n/seasasfs01/data/geos/GEOS_NATIVE/MAP_A2A_Regrid_201203/MAP_A2A_latlon_geos025x03125.nc')

    CALL READ_INPUT_GRID(1152, 721, gridfile, LON_EDGES, LAT_SINES)
    ...
    DO I = 1, 1152
        IF (II <= IIPAR) THEN
            IF (XEDGE(II+1,10,1) > LON_EDGES(I)) THEN
                IF (QHR(I,J) > maxpbl) THEN
                maxpbl=QHR(I,J)
                ENDIF
            ELSE
                Qmid(II,J) = maxpbl
            ...
            ENDIF
        ELSE
            Qmid(1,J) = max(Qmid(1,J), QHR(I,J))
        ENDIF
    ENDDO
    Qmid(1,J) = max(Qmid(1,J), maxpbl)
    maxpbl = 0
    II = 1
    ENDDO
    ...
    DO I = 1, IIPAR
    DO J = 1, 721
        IF (YSIN(10,JJ+1,1) > LAT_SINES(J)) THEN
            IF (Qmid(I,J) > maxpbl) THEN
                maxpbl = Qmid(I,J)
            ENDIF
        ELSE
            Q(I,JJ) = maxpbl
    ...
    !State_Met%PBLH_MAX = Q
    State_Met%PBLH_MAX = State_Met%PBLH
...
!
 ...
 20    FORMAT( '     - GEOS-FP A3 met fields for ', a,  &
    ...
    CALL GeosFp_Read_A3dyn_native( YYYYMMDD, HHMMSS, Input_Opt, State_Met )
    CALL GeosFp_Read_A3mstC_native( YYYYMMDD, HHMMSS, Input_Opt, State_Met )
    CALL GeosFp_Read_A3mstE_native( YYYYMMDD, HHMMSS, Input_Opt, State_Met )
    ELSE
...
!BOP
!
! !IROUTINE: geosfp_read_a3cld_native
!
! !DESCRIPTION: Routine to read variables and attributes from a GEOS-FP
!  met fields file containing 3-hr time-averaged (A3) data (cloud fields).
!\\
...
! !INTERFACE:
!
  SUBROUTINE GeosFp_Read_A3cld_native( YYYYMMDD, HHMMSS, Input_Opt, State_Met )
!
! !USES:
!
    USE GIGC_Input_Opt_Mod, ONLY : OptInput
...
!
    ...
    INTEGER            :: st4d(4), ct4d(4)         ! Start & count indices
    REAL*4             :: Q(IIPAR,JJPAR,LGLOB)     ! Temporary data arrray
    !REAL*4             :: Q1(IIPAR,JJPAR,LGLOB)     ! Temporary data arrray
    ...
    ! Name of this routine (for error printout)
    caller  = "GEOSFP_READ_A3cld_native (geosfp_read_mod.F90)"
    ...
    ! Replace time & date tokens in the file name
    nc_file = 'GEOSFP.YYYYMMDD.A3cld.Native.nc' 
    !nc_file = 'GEOS.fp.asm.tavg3_3d_cld_Nv.YYYYMMDD_hhmm.V01.nc4' 
    CALL EXPAND_DATE( nc_file, YYYYMMDD, HHMMSS )
 ...
 100   FORMAT( 'Time_index value ', i5, ' must be in the range 1 to 8!' )
    ...
    !CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    !v_name = 'QILS'
    !CALL NcRd( Q1, fId, TRIM(v_name), st4d, ct4d )
    !CALL Transfer_3d( (Q(:,:,LGLOB:1:-1) + Q1(:,:,LGLOB:1:-1)), State_Met%QI )
    State_Met%QI = 0
    ...
    !v_name = "QLLS"
    !CALL NcRd( Q1, fId, TRIM(v_name), st4d, ct4d )
    !CALL Transfer_3d( (Q(:,:,LGLOB:1:-1) + Q1(:,:,LGLOB:1:-1)), State_Met%QL )
    State_Met%QL = 0

    ! Read TAUCLI
    v_name = "TAUCLI"
    !CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    !CALL Transfer_3d( Q(:,:,LGLOB:1:-1), State_Met%TAUCLI )
    State_Met%TAUCLI = 0

    ! Read TAUCLW
    v_name = "TAUCLW"
    !CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    !CALL Transfer_3d( Q(:,:,LGLOB:1:-1), State_Met%TAUCLW )
    State_Met%TAUCLW = 0
 ...
 10 FORMAT( '     - Found all 6  GEOS-FP A3cld  met fields for ', a )
  ...
  END SUBROUTINE GeosFp_Read_A3cld_native
!EOC
!------------------------------------------------------------------------------
!                  GEOS-Chem Global Chemical Transport Model                  !
!------------------------------------------------------------------------------
...
!
 ...
 100   FORMAT( 'Time_index value ', i5, ' must be in the range 1 to 8!' )
    ...
    CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d( Q, State_Met%DTRAIN )
    !State_Met%DTRAIN = State_Met%DTRAIN * 2.0d0
...
!BOP
!
! !IROUTINE: geosfp_read_a3dyn_native
!
! !DESCRIPTION: Routine to read variables and attributes from a GEOS-FP
!  met fields file containing 3-hr time-averaged (A3) data (dynamics fields).
!\\
...
! !INTERFACE:
!
  SUBROUTINE GeosFp_Read_A3dyn_native( YYYYMMDD, HHMMSS, Input_Opt, State_Met )
!
! !USES:
!
    USE GIGC_Input_Opt_Mod, ONLY : OptInput  
...
!
    ...
    ! Name of this routine (for error printout)
    caller  = "GEOSFP_READ_A3dyn_native (geosfp_read_mod.F90)"
    ...
    ! Replace time & date tokens in the file name
#if defined(GRID025x03125) || defined( GRID025x025 ) 
    nc_file = 'Native.nc'
#else
    nc_file = Get_Resolution_String()
#endif
 ...
 100   FORMAT( 'Time_index value ', i5, ' must be in the range 1 to 8!' )
    ...
    CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    CALL TRANSFER_3d( Q, STate_Met%DTRAIN )
    !State_Met%DTRAIN = State_Met%DTRAIN * 2.0d0   ! adjust convection
    ...
    stamp = TimeStamp_String( YYYYMMDD, HHMMSS )
    WRITE( 6, 10 ) stamp
 10 FORMAT( '     - Found all 6  GEOS-FP A3dyn native met fields for ', a )
  ...
  END SUBROUTINE GeosFp_Read_A3dyn_native
!EOC
!------------------------------------------------------------------------------
!                  GEOS-Chem Global Chemical Transport Model                  !
!------------------------------------------------------------------------------
...
!
    ...
    ! Make sure the dimension of the file are valid
    !CALL Check_Dimensions( lon=X,  lat=Y,           lev=Z,         &
    !                       time=T, time_expected=8, caller=caller )
...
!BOP
!
! !IROUTINE: geosfp_read_a3mstc_native
!
! !DESCRIPTION: Routine to read variables and attributes from a GEOS-FP
!  met fields file containing 3-hr time-averaged (A3) data (moist fields,
!  saved on level centers).
...
! !INTERFACE:
!
  SUBROUTINE GeosFp_Read_A3mstC_native( YYYYMMDD, HHMMSS, Input_Opt, State_Met )
!
! !USES:
!
    USE GIGC_Input_Opt_Mod, ONLY : OptInput
...
!
    ...
    ! Name of this routine (for error printout)
    caller  = "GEOSFP_READ_A3mstC_native (geosfp_read_mod.F90)"
 ...
 10 FORMAT( '     - Found all 4  GEOS-FP A3mstC met fields for ', a )
  ...
  END SUBROUTINE GeosFp_read_A3mstC_native
!EOC
!------------------------------------------------------------------------------
!                  GEOS-Chem Global Chemical Transport Model                  !
!------------------------------------------------------------------------------
...
!    State_Met%CMFMC = State_Met%CMFMC * 2.0d0

    ! Read PFICU
    v_name = "PFICU"
    CALL NcRd( Qe, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d_Lp1( Qe, State_Met%PFICU )
!    State_Met%PFICU = State_Met%PFICU * 2.0d0
...
!    State_Met%PFLCU = State_Met%PFLCU * 2.0d0
...
!BOP
!
! !IROUTINE: geosfp_read_a3mste_native
!
! !DESCRIPTION: Routine to read variables and attributes from a GEOS-FP
!  met fields file containing 3-hr time-averaged (A3) data (moist fields,
!  saved on level edges).
...
! !INTERFACE:
!
  SUBROUTINE GeosFp_read_A3mstE_native( YYYYMMDD, HHMMSS, Input_Opt, State_Met )
!
! !USES:
!
    USE GIGC_Input_Opt_Mod, ONLY : OptInput
...
!  07 Feb 2012 - R. Yantosca - Now echo info after reading fields from disk
!  10 Feb 2012 - R. Yantosca - Now get a string for the model resolution
!  09 Nov 2012 - M. Payer    - Copy all met Input_Opt%DATA_DIRfields to the State_Met derived type
!                              object
!  15 Nov 2012 - R. Yantosca - Now replace dao_mod.F arrays with State_Met
!  11 Apr 2013 - R. Yantosca - Now pass directory fields with Input_Opt
!  26 Sep 2013 - R. Yantosca - Renamed to GeosFp_Read_A3mstE
...
!
    ...
    ! Name of this routine (for error printout)
    caller  = "GEOSFP_READ_A3mstE_native (geosfp_read_mod.F90)"
 ...
 100   FORMAT( 'Time_index value ', i5, ' must be in the range 1 to 8!' )
    ...
    CALL NcRd( Qe, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d_Lp1( Qe, State_Met%CMFMC )
    !State_Met%CMFMC = State_Met%CMFMC * 2.0d0    ! adjust convection

    ! Read PFICU
    v_name = "PFICU"
    CALL NcRd( Qe, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d_Lp1( Qe, State_Met%PFICU )
    !State_Met%PFICU = State_Met%PFICU * 2.0d0   ! adjust convection
    ...
    CALL NcRd( Qe, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d_Lp1( Qe, State_Met%PFLCU )
    !State_Met%PFLCU = State_Met%PFLCU * 2.0d0   ! adjust convection
 ...
 10 FORMAT( '     - Found all 4  GEOS-FP A3mstE met fields for ', a )
  ...
  END SUBROUTINE GeosFp_Read_A3mstE_native
!EOC
!------------------------------------------------------------------------------
!                  GEOS-Chem Global Chemical Transport Model                  !
!------------------------------------------------------------------------------
!BOP
!
! !ROUTINE: geosfp_read_omega
!
! !DESCRIPTION: Routine to read up and down vertical fluxes separately
!\\
!\\
! !INTERFACE:
!
  SUBROUTINE GeosFp_Read_OMEGA( YYYYMMDD, HHMMSS, Input_Opt, State_Met )
!
! !USES:
!
    USE GIGC_Input_Opt_Mod, ONLY : OptInput
...
!
!  !REVISION HISTORY:
!  07 Mar 2014  - K. Yu     - Initial version
!EOP
!------------------------------------------------------------------------------
!BOC
!
...
!
    ...
    INTEGER             :: X, Y, Z, T       ! netCDF file dimensions
    INTEGER             :: time_index       ! Read this slice of data
    CHARACTER(LEN=16)   :: stamp            ! Time and date state
    CHARACTER(LEN=255)  :: nc_file          ! netCDF file name
    CHARACTER(LEN=255)  :: v_name           ! netCDF variable name
    CHARACTER(LEN=255)  :: dir              ! Data directory path
    CHARACTER(LEN=255)  :: errMsg           ! Error message
    ...
    ! Arrays
    INTEGER             :: st4d(4), ct4d(4) ! Start and count indices
    REAL*4              :: Q(IIPAR, JJPAR, LGLOB)
    ...
    IF ( time_index < 1 .or. time_index > T ) THEN
        WRITE( 6, 100 ) time_index
100     FORMAT( 'Time_index_value', i5, ' must be in the range 1 to 8!' )
        CALL Error_Stop( errMsg, caller )
    ENDIF
    ...
    v_name = 'negOmega'
    CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d( Q, State_Met%negomega)

    ! Echo info
    stamp = TimeStamp_String( YYYYMMDD, HHMMSS )
    WRITE( 6, 10 ) stamp
 10 FORMAT('               - Fount all 2 GEOS-FP OMEGA met fields for ', a )
...
!BOP
!
! !ROUTINE: geosfp_read_omega_native
!
! !DESCRIPTION: Routine to read up and down vertical fluxes separately
!\\
!\\
! !INTERFACE:
!
  SUBROUTINE GeosFp_Read_OMEGA_native( YYYYMMDD, HHMMSS, Input_Opt, State_Met )
!
! !USES:
!
    USE GIGC_Input_Opt_Mod, ONLY : OptInput
...
!
    ...
    CHARACTER(LEN=255)  :: errMsg           ! Error message
    CHARACTER(LEN=255)  :: caller           ! NAme of this routine
    CHARACTER(LEN=255)  :: gridfile         ! filename of grid 

    ! Arrays
    INTEGER             :: st4d(4), ct4d(4) ! Start and count indices
    INTEGER            :: st3d(3), ct3d(3)         ! Start & count indices
    REAL(fp), TARGET      :: Q(1152, 721, LGLOB)
    REAL(fp), TARGET      :: Q2(1152, 721)
    REAL(fp), TARGET      :: NegOmega(1152, 721, LGLOB)
    REAL(fp), TARGET      :: Qremap(IIPAR, JJPAR, LGLOB)
    REAL(fp), POINTER   :: INGRID(:,:)
    REAL(fp), POINTER   :: OUTGRID(:,:)
    REAL*4              :: Q4(IIPAR, JJPAR, LGLOB)
    ...
    ! Replace time & date tokens in the file name
    nc_file = 'GEOSFP.YYYYMMDD.A3dyn.Native.nc' 
    CALL EXPAND_DATE( nc_file, YYYYMMDD, HHMMSS )
...
100     FORMAT( 'Time_index_value', i5, ' must be in the range 1 to 8!' )
    ...
    stamp = TimeStamp_String( YYYYMMDD, HHMMSS )
    WRITE( 6, 10 ) stamp
 10 FORMAT('               - Fount GEOS-FP OMEGA met fields for ', a )

    ! Close netCDF file
    CALL NcCl( fId )

    ! regrid omega by positive and negatives
    print*, 'Online regridding of pos and neg omegas'
    NegOmega = Q
    WHERE ( NegOmega > 0 ) NegOmega = 0e0

    gridfile = TRIM('/n/seasasfs01/data/geos/GEOS_NATIVE/MAP_A2A_Regrid_201203/MAP_A2A_latlon_geos025x03125.nc')
    DO L = 1, LGLOB
        INGRID => Q(:,:,L)
        OUTGRID => Qremap(:,:,L)
        CALL DO_REGRID_A2A( gridfile, 1152, 721, INGRID, OUTGRID, 0)
    ENDDO
    Q4 = Qremap
    CALL Transfer_3d( Q4, State_Met%OMEGA)
    ...
    ENDDO
    Q4 = Qremap
    CALL Transfer_3d( Q4, State_Met%negomega)

  END SUBROUTINE GeosFp_Read_OMEGA_native
!EOC
...
!
    ...
    ! Replace time & date tokens in the file name
#if defined(GRID025x03125) && !defined(NEST_NA) && !defined(NEST_EU) && !defined(NEST_CH) 
    nc_file = 'Native.nc'
#elif defined(GRID025x025)
    nc_file = 'Native.nc'
#else
