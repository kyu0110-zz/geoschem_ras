...
!
    ...
    dir     = TRIM( 'GEOS_FP/2015/')
    nc_file = 'GEOS.fp.asm.const_2d_asm_Nx.00000000_0000.V01.nc4'
    nc_file = TRIM( '/n/regal/jacob_lab/kyu/025_MET/GEOS_FP/') // TRIM(nc_file)
#else
...
!
 ...
 20    FORMAT( '     - GEOS-FP A1 met fields for ', a,  &
    ...
    nc_file = 'GEOS.fp.asm.tavg1_2d_flx_Nx.YYYYMMDD_hhmm.V01.nc4'
#else
    nc_file = Get_Resolution_String()
...
#if defined( GRID025x03125 ) && !defined( NEST_NA ) && !defined( NEST_CH ) && !defined( NEST_EU )
    ! Don't read in ALBEDO, CLDTOT, FRSEAICE
    State_Met%ALBD = 0
    State_Met%CLDFRC = 0
...
#if defined( GRID025x03125 ) && !defined( NEST_NA) && !defined( NEST_CH) && !defined( NEST_EU )
    CALL GeosFp_Read_A3cld_native( YYYYMMDD, HHMMSS, Input_Opt, State_Met )
...
!
 ...
 100   FORMAT( 'Time_index value ', i5, ' must be in the range 1 to 8!' )
    ...
    v_name = 'DTRAIN'
    CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    CALL TRANSFER_3d( Q(:,:,LGLOB:1:-1), STate_Met%DTRAIN )
...
!
 ...
 100   FORMAT( 'Time_index value ', i5, ' must be in the range 1 to 8!' )
    ...
    v_name = "RH"
    CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d( Q(:,:,LGLOB:1:-1), State_Met%RH )

    ! Read U
    v_name = "U"
    CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d( Q(:,:,LGLOB:1:-1), State_Met%U )

    ! Read V
    v_name = "V"
    CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d( Q(:,:,LGLOB:1:-1), State_Met%V )
...
!
    ...
    ! Replace time & date tokens in the file name
    nc_file = 'GEOS.fp.asm.tavg3_3d_mst_Nv.YYYYMMDD_hhmm.V01.nc4'
    CALL EXPAND_DATE( nc_file, YYYYMMDD, HHMMSS )
 ...
 100   FORMAT( 'Time_index value ', i5, ' must be in the range 1 to 8!' )
    ...
    v_name = "DQRCU"
    CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d( Q(:,:,LGLOB:1:-1), State_Met%DQRCU )

    ! Read DQRLSAN
    v_name = "DQRLSAN"
    !CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    !CALL Transfer_3d( Q(:,:,LGLOB:1:-1), State_Met%DQRLSAN )
    State_Met%DQRLSAN = 0

    ! Read REEVAPCN
    v_name = "REEVAPCN"
    CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d( Q(:,:,LGLOB:1:-1), State_Met%REEVAPCN )

    ! Read  from file
    v_name = "REEVAPLSAN"
    !CALL NcRd( Q, fId, TRIM(v_name), st4d, ct4d )
    !CALL Transfer_3d( Q(:,:,LGLOB:1:-1), State_Met%REEVAPLS )
    State_Met%REEVAPLS = 0
...
!
    ...
    ! Replace time & date tokens in the file name
    nc_file = 'GEOS.fp.asm.tavg3_3d_mst_Ne.YYYYMMDD_hhmm.V01.nc4'
    CALL EXPAND_DATE( nc_file, YYYYMMDD, HHMMSS )
 ...
 100   FORMAT( 'Time_index value ', i5, ' must be in the range 1 to 8!' )
    ...
    v_name = "CMFMC"
    CALL NcRd( Qe, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d_Lp1( Qe(:,:,LGLOB+1:1:-1), State_Met%CMFMC )

    print*, 'read cmfmc'
    ! Read PFICU
    v_name = "PFICU"
    CALL NcRd( Qe, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d_Lp1( Qe(:,:,LGLOB+1:1:-1), State_Met%PFICU )

    print*, 'read pficu'
    ! Read PFILSAN
    v_name = "PFILSAN"
    !CALL NcRd( Qe, fId, TRIM(v_name), st4d, ct4d )
    !CALL Transfer_3d_Lp1( Qe(:,:,LGLOB+1:1:-1), State_Met%PFILSAN )
    State_Met%PFILSAN = 0.0

    print*, 'read pfilsan'
    ! Read PFLCU
    v_name = "PFLCU"
    CALL NcRd( Qe, fId, TRIM(v_name), st4d, ct4d )
    CALL Transfer_3d_Lp1( Qe(:,:,LGLOB+1:1:-1), State_Met%PFLCU )

    ! Read  from file
    v_name = "PFLLSAN"
    !CALL NcRd( Qe, fId, TRIM(v_name), st4d, ct4d )
    !CALL Transfer_3d_Lp1( Qe(:,:,LGLOB+1:1:-1), State_Met%PFLLSAN )
    State_Met%PFLLSAN = 0.0
...
!
    ...
    ! Replace time and date tokens in the file name
...
!
 ...
 100   FORMAT( 'Time_index value ', i5, ' must be in the range 1 to 8!' )
    ...
    CALL NcRd( Q3, fId, TRIM(v_name), st4d, ct4d )
#if defined(GRID025x03125) && !defined(NEST_NA) && !defined(NEST_EU) && !defined(NEST_CH)
    CALL Transfer_3d( Q3(:,:,LGLOB:1:-1), State_Met%SPHU1)
#else 
    ...
    CALL NcRd( Q3, fId, TRIM(v_name), st4d, ct4d )
#if defined(GRID025x03125) && !defined(NEST_NA) && !defined(NEST_EU) && !defined(NEST_CH)
    CALL Transfer_3d( Q3(:,:,LGLOB:1:-1), State_Met%TMPU1)
#else
...
!
 ...
 100   FORMAT( 'Time_index value ', i5, ' must be in the range 1 to 8!' )
    ...
    CALL NcRd( Q3, fId, TRIM(v_name), st4d, ct4d )
#if defined(GRID025x03125) && !defined(NEST_NA) && !defined(NEST_EU) && !defined(NEST_CH)
    CALL Transfer_3d( Q3(:,:,LGLOB:1:-1), State_Met%SPHU2 )
#else
    ...
    CALL NcRd( Q3, fId, TRIM(v_name), st4d, ct4d )
#if defined(GRID025x03125) && !defined(NEST_NA) && !defined(NEST_EU) && !defined(NEST_CH)
    CALL Transfer_3d( Q3(:,:,LGLOB:1:-1), State_Met%TMPU2 )
#else
