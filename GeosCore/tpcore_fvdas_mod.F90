...
!
    ...
    REAL(fp),  INTENT(INOUT) :: OMEGAFLUX(:,:,:)
    REAL(fp),  INTENT(INOUT) :: OMEGAINST(:,:,:)
!
! !OUTPUT PARAMETERS:
!
...
!c?
             ...
             jv  = rij - va(il,ij)
                ...
                ! kludge so we don't get out of bounds errors 
             !if (abs(va(il, ij)) > 2) then
                !if (mod(il, 500) == 0) print*, 'rij, va, jv', rij, va(il, ij), jv
                !jv = rij - sign(2.0, va(il,ij)) 
             !endif
                !!!! kyu 
             qqv(il,ij) =  &
                  va(il,ij) *  &
...
!c?
                ...
                ! tmp kludge 
             !if (abs(va(il, ij)) > 2) then
              !  if (mod(il, 500) == 0) print*, 'ij, va, jv', ij, va(il, ij), jv
               ! jv = ij - sign(2.0, va(il,ij))
             !else 
                jv = ij - jv
             !endif
                !!!
...
!   =============
    ...
    !do ij = 1, 720
    !do ik = 1, 72
    !    if (abs(2*qq1(il,ij,ik)) < abs(fz(il,ij,ik))) then
    !        print*, 'fz exceeds qq1, fz = ', fz(il,ij,ik), 'qq1=', qq1(il,ij,ik)
    !    endif
    !enddo
    !enddo
    !enddo
