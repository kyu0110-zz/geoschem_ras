...
!
    ...
    ! Physical constants
    REAL(fp), PARAMETER ::  MAPL_GRAV = 9.80665     !m^2/s
    REAL(fp), PARAMETER  ::   MAPL_RUNIV = 8314.47    !J/Kmol K
    REAL(fp), PARAMETER ::  MAPL_AIRMW = 28.965     !kg/Kmol
    REAL(fp), PARAMETER ::  MAPL_H2OMW = 18.015
    REAL(fp), PARAMETER ::  MAPL_RDRY = MAPL_RUNIV/MAPL_AIRMW
    REAL(fp), PARAMETER ::  MAPL_CPDRY = 3.5 * MAPL_RDRY
    ...
    REAL(fp), PARAMETER ::  MAPL_ALHL   = 2.4665E6               ! J/kg @15C
    REAL(fp), PARAMETER ::  MAPL_EPSILON = MAPL_H2OMW/MAPL_AIRMW
    REAL(fp), PARAMETER ::  MAPL_VIREPS = 1.0/MAPL_EPSILON - 1.0
...
!
      ...
      TYPE, PUBLIC :: RASPARAM_TYPE
        ...
        REAL(fp)        :: LAMBDA_FAC
        REAL(fp)        :: LAMBMX_FAC
        REAL(fp)        :: MIN_DIAMETER
        REAL(fp)        :: CUFRICLAMBDA
        REAL(fp)        :: RDTLEXPON
        REAL(fp)        :: STRAPPING
        REAL(fp)        :: SDQV2
        REAL(fp)        :: SDQV3
        ...
        REAL(fp)        :: LLDISAGGXP
        REAL(fp)        :: PBLFRAC
        REAL(fp)        :: RASAUTORAMPB
        REAL(fp)        :: AUTOC_CN_ZDEP
        REAL(fp)        :: MAXDALLOWED_S
        REAL(fp)        :: MAXDALLOWED_D
        INTEGER         :: RASAL_EXP
        REAL(fp)        :: RAS_RHMIN
        REAL(fp)        :: RAS_RHFULL
...
!
! !REVISION HISTORY:
! 23 Feb 2017 - K. Yu - Initial version taken from strippedras.F90 and
! qsat.F90 from Andrea Molod. 
!EOP
!-------------------------------------------------------------------------
!BOC
      CONTAINS
...
!\\
!
      SUBROUTINE INIT_RASPARAMS( am_I_Root, RASPARAMS, RC )
!
! !USES:
!
      USE GIGC_ErrCode_Mod
...
!
      ...
      RASPARAMS%CUFRICFAC = 1.000_fp
      RASPARAMS%SHR_LAMBDA_FAC = 0.05_fp
      RASPARAMS%QC_CRIT_CN = 8.0e-4
      RASPARAMS%RASAL2 = -43200.0_fp
      RASPARAMS%RASNCL = -300.0_fp
      RASPARAMS%LAMBDA_FAC = 4.0
      RASPARAMS%LAMBMX_FAC = 0.0_fp
...
!BOP
!
! !DESCRTION: Wrapper for RASE
!\\
!\\
!
      SUBROUTINE DO_RAS( am_I_Root, Input_Opt,      &
                         State_Met, RASPARAMS, RC )
!
! !USES:
! 
      USE CMN_DIAG_MOD
...
!
    ...
    ENDDO
      ...
      DO J = 1, JJPAR
      DO I = 1, IIPAR
      TMP_INT(I,J) = 1000000 * ( 100*TEMP(I,J,LLPAR)   - INT( 100*TEMP(I,J,LLPAR) ) ) 
      ENDDO
      ENDDO
      ! Rearrange to 1-d
      ...
      DO J = 1, JJPAR
      DO I = 1, IIPAR
      TMP_INT(I,J) = 1000000 * ( 100*TEMP(I,J,LLPAR-1) - INT( 100*TEMP(I,J,LLPAR-1) ) ) 
      ENDDO
      ENDDO
      ! Rearrange to 1-d
    ...
    DO I = 1, IIPAR
      ...
      IRAS((J-1)*IIPAR + I) = NINT(LON / 360.0 * 2.0 * PI * 100.0)
      JRAS((J-1)*IIPAR + I) = NINT(GET_YMID(I,J,1) / 360.0 * 2.0 * PI * 100.0)
    ENDDO
    ENDDO
    ...
    DO I = 1, IDIM
            WGT0(I,:)            = 0.
            WGT0(I, KCBL(I):LLPAR) = 1.0
            WGT1(I,:)           = 0.
            WGT1(I, KCBL(I):LLPAR) = 1.0
      ENDDO 
    ...
    DO J = 1, JJPAR
    DO I = 1, IIPAR
      TH(I,J,L) = TEMP(I,J,L) * (State_Met%PSC2(I,J) / PMID_DRY(I,J,L)) ** MAPL_KAPPA
    ENDDO
    ENDDO
    ENDDO
    ...
    PLO = 0.5*(CNV_PLE(:,1:LLPAR) + CNV_PLE(:,2:LLPAR+1))
    PK = (PLO/1000.0) ** (MAPL_RGAS/MAPL_CP)
    PKE = (CNV_PLE/1000.0) ** (MAPL_RGAS/MAPL_CP)

    print*, 'PLO min max mean', minval(PLO), maxval(PLO),sum(PLO)/(IDIM*LLPAR)
    print*, 'PKE min max mean', minval(PKE), maxval(PKE),sum(PKE)/(IDIM*LLPAR+IDIM)

    DO L = 1, LLPAR
      CALL REARRANGE_GRID(Q(:,:,L), QHO(:,L))
    ENDDO
    QHO = QHO * 0.001    ! convert from g/kg to kg/kg

    ZLE(:,LLPAR+1) = 0
      do L=LLPAR,1,-1
      DO I = 1, IDIM
         ZLE(I,L) = THO(I,L) * (1.+MAPL_VIREPS*QHO(I,L))
         ZLO(I,L) = ZLE(I,L+1) + (MAPL_CP/MAPL_GRAV)*(PKE(I,L+1)-PK (I,L) ) * ZLE(I,L)
         ZLE(I,L) = ZLO(I,L) + (MAPL_CP/MAPL_GRAV)*( PK (I,L)-PKE(I,L) ) * ZLE(I,L)
      ENDDO
      end do
    ...
    print*, 'ZLE 1 min max mean', minval(ZLE(:,LLPAR)), maxval(ZLE(:,LLPAR)), sum(ZLE(:,LLPAR))/IDIM
    print*, 'ZLE 50min max mean', minval(ZLE(:,50)), maxval(ZLE(:,50)), sum(ZLE(:,50))/IDIM
    print*, 'KCBL  min max mean', minval(KCBL ), maxval(KCBL ), sum(KCBL)/IDIM
    print*, 'ZCBLx min max mean', minval(ZCBLx), maxval(ZCBLx), sum(ZCBLx)/IDIM
    ...
    DO I = 1, IIPAR
      DO L = 1, LLPAR
        ...
        IF (EDGE_PRES(I,J) > 600.0) EXIT
      ENDDO
    ENDDO
    ENDDO
    QV600 = QV600 / 1000.0d0

    IF ( CNV_Q600_MAX > CNV_Q600_MIN ) THEN
      DO J = 1, JJPAR
      DO I = 1, IIPAR
        TMP_REAL(I,J) = MAX(0.0, MIN(1.0, (QV600(I,J) - &
    ...
    if (RASPARAMS%RASAL2 > 0.0) then
      ...
      DO I=1, IDIM
        RASAL2_2d(I) = CNV_FRAC(I)*ABS(RASPARAMS%RASAL2) + (1-CNV_FRAC(I))*RASPARAMS%RASAL1
      END DO
    endif
    ...
    !------------------------------------------------------------------------
      CALL RASE(                  &                    
           ...
           PKE                  , &
           ! outputs
           CNV_FLX              , &   ! -> progno_cloud
           CNV_FLXD             , &   ! -> diag
           CNV_FLXC             , &   ! -> progno_cloud
           RASPARAMS            , & ! params
           IRC                 )
        ...
        DO J = 1, JJPAR
            ...
            IF (COLUMN_RAS < 1e-2 .or. abs(COLUMN_RAS - COLUMN_FP)/COLUMN_RAS < 0.25) THEN
                PART(I,J) = 0.0
            ELSE
                part(I,J) = 1.0 - (COLUMN_FP/COLUMN_RAS)
            ENDIF
        ENDDO
        ENDDO
        ...
        DO I = 1, IIPAR
            part_tmp = PART(I,J)
            if (part_tmp >= 0) then
                repartition_dqr = min(part_tmp * State_Met%DQRLSAN(I,J,L), State_Met%DQRLSAN(I,J,L))
                repartition_pfi = min(part_tmp * State_Met%PFILSAN(I,J,L), State_Met%PFILSAN(I,J,L))
            else
                repartition_dqr = max(part_tmp * State_Met%DQRLSAN(I,J,L), -1.0*State_Met%DQRCU(I,J,L))
                repartition_pfi = max(part_tmp * State_Met%PFILSAN(I,J,L), 0.0)!-1.0*State_Met%PFICU(I,J,L))
            endif

            ! Repartition DQR
            State_Met%RAS_DQRCU(I,J,L) = State_Met%DQRCU(I,J,L) !+ repartition_dqr 
            State_Met%RAS_DQRLSAN(I,J,L) = State_Met%DQRLSAN(I,J,L) !- repartition_dqr

            ! Repartition PFI
            State_Met%RAS_PFICU(I,J,L) = State_Met%PFICU(I,J,L) !+ repartition_pfi
            State_Met%RAS_PFILSAN(I,J,L) = State_Met%PFILSAN(I,J,L) !- repartition_pfi

            if (part_tmp >= 0) then
                repartition = min(part_tmp * State_Met%PFLLSAN(I,J,L), State_Met%PFLLSAN(I,J,L))
            else
                repartition = max(part_tmp * State_Met%PFLLSAN(I,J,L), 0.0) !-1.0*State_Met%PFLCU(I,J,L))
            endif
            ...
            if (part_tmp >= 0) then
                repartition = min(part_tmp * State_Met%REEVAPLS(I,J,L), 0.5*State_Met%REEVAPLS(I,J,L))
            else
                repartition = max(part_tmp * 0.5*State_Met%REEVAPLS(I,J,L), 0.0)! -1.0*State_Met%REEVAPCN(I,J,L))
            endif
        ...
        ENDDO
        ENDDO
            print*, 'max DQR', maxval(State_Met%PFICU), maxval(State_Met%RAS_PFICU)
            print*, 'min DQR', minval(State_Met%PFICU), minval(State_Met%RAS_PFICU)
            print*, 'max REEVAP', maxval(State_Met%REEVAPCN), maxval(State_Met%RAS_REEVAPCN)
            print*, 'min REEVAP', minval(State_Met%REEVAPCN), minval(State_Met%RAS_REEVAPCN)
        !------------------------------------------------------------------
        ! Write to diagnostic
        !------------------------------------------------------------------
